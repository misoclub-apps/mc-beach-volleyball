...
BEACH NOTEは [misoclub](https://misoclub.pro/) が制作・運営しています。
...
JBV公式選手名鑑に写真が掲載されている選手は、個別プロフィールページの本人写真へ直接リンクして一覧・選手ページに表示します。公式画像の縦長比率を活かして表示し、個別ページの画像を確認できない場合は名鑑一覧の画像、それも取得できない場合は選手名の頭文字へ戻します。
...
原本は `.cache/http/` に保存されます。本文ハッシュ、ETag、Last-Modified、取得日時も保存します。同じURLのPDF差し替えも検知できるよう毎回再確認します。1リクエストあたり最低1.2秒、1実行あたり記事160件・名簿PDF160件に制限しています。HTTP 408・429・5xxや一時的な通信失敗は、待機時間を広げながら最大5回リトライします。robots.txtの制限がある場合は尊重し、取得できなかった場合はレポートに記録します。初回確認ではJBVのrobots.txtはHTTP 500でした。再利用条件の確認状況は `docs/source-review-2026-09-22.md` を参照してください。
